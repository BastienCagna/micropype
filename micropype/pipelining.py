...
def get_versions():
    return {"python": f"{sys.version[0]}.{sys.version[1]}"}
...
def run_cmd(cmd, title=None, log=None, versions: dict = None, raise_errors=True):
    ...
    try:
        ...
        if log:
            write_log(log, f"Errored at {datetime.isoformat(datetime.now())}:\n", str(e), "\n#####\n\n\n")
        if raise_errors:
            raise e
        else:
...
def _run_cmd(splitted_cmd):
    ...
    process = subprocess.Popen(
        splitted_cmd, stdout=subprocess.PIPE, shell=True, stderr=subprocess.STDOUT
...
def cached_run(
    ...
    if isinstance(out_files, str) or isinstance(out_files, Path):
        out_files = [out_files]
