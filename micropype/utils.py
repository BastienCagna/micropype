...
def check_dir(*args, dir_exist_ok=True) -> str:
    ...
    # if path is file path like
    if "." in op.split(path)[1]:
...
def cprint(*args, intent: MessageIntent = MessageIntent.DEFAULT):
    ...
    if intent == MessageIntent.INFO:
        clr = Fore.BLUE
    elif intent == MessageIntent.SUCCESS:
        clr = Fore.GREEN
    elif intent == MessageIntent.ERROR:
        clr = Fore.RED
    elif intent == MessageIntent.WARNING:
        clr = Fore.YELLOW
    elif intent == MessageIntent.HIGHLIGHT:
